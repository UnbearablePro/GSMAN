class ContacteEventHandler {
  ...
  static handleEvent(event) {
    if (DataUtils.isEmpty(event)) {
      throw new Error("Event holder does not exist");
    } else {
  ...
  static checkForStatusEvent(event) {
    if (event.row > event.indexes.StartingDataRow &&
      event.col == event.headers.Status) {
      ContacteStatusHandler.handleStatusEvent(event);
      return;
    }
  }
}
