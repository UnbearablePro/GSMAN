...
class ContacteSheet {
  ...
  static initialize_() {
    if (DataUtils.isEmpty(ContacteSheet.sheet)) {
      this.sheet = SheetController.getSheetByName(ContacteSheet.name);
    }
  }
  ...
  static updateInteractionDate(row = SheetController.getCurrentCell()) {
    SheetController.setValue(this.sheet, DateUtils.toDay(), row,  this.headers.Status);
  }
  ...
  static setDataValidationFor(range, referenceValueToSetValidators) {
    SheetController.setDataValidation(range, referenceValueToSetValidators, ContacteStatusHandler.getContacteValidationRuleFunction);
  }

  static setValue(row, col, value) {
    this.initialize_();
    SheetController.setValue(ContacteSheet.sheet, value, row, col);
  }
